...
describe('корректировка в интерфейсе', () => {
  ...
  it('в CSV выгружается со знаком по направлению и со счётом', () => {
    ...
    expect(rows[0]).toBe('2026-09-20,Корректировка,-213.00,UAH,,Карта,,,Сверка с банком')
    expect(rows[1]).toBe('2026-09-20,Корректировка,50.00,UAH,,Карта,,,Сверка с банком')
  })
})
