...
/**
 * Потолок для явного досоздания за паузу и для подсчёта пропущенного.
 *
 * Здесь лимит одного прохода не годится: пользователю показывают число
 * пропущенных платежей и кнопку «Создать», поэтому назвать 400 вместо 628 —
 * значит соврать. Досоздание — редкое и осознанное действие, не запуск
 * приложения, так что несколько сотен лишних записей можно позволить.
 */
export const MAX_BACKFILL_OCCURRENCES = 5_000
